...
class Quantizer(nn.Module):
    def __init__(self, mode="base", bit=8, is_signed=True, is_enable=False, is_input=False, args=None, operator=None,
                 nestbit=4, nest=True, adaround=True):
        ...
        if not self.is_input:
            ...
            self.nestbit = nestbit
            self.shiftbit = self.bit - self.nestbit
            assert self.shiftbit > 0

            self.scale_nest = None
            self.zero_point_nest = None
            self.quant_high_int = None
            self.quant_low_int = None
    ...
    def _init_quant_para(self, data):
        if self.has_inited_quant_para == False:
            ...
            if self.mode == "squant_-e":
                self.squant_k = False
                self.squant_c = False
                self.mode = "squant_"
            elif self.mode == "squant_-k":
                self.squant_c = False
                self.mode = "squant_"
            elif self.mode == "squant_-c":
                self.squant_k = False
                self.mode = "squant_"
            
            if self.mode == "squant_":
                def _quant(tensor):
                    ...
                    self.scale, self.zero_point = asymmetric_linear_quantization_params(self.bit, x_min, x_max)
                    quant_tensor = linear_quantize(tensor, self.scale, self.zero_point, inplace=False)
                    ...
                    self.quant_int = quant_tensor

                    n_high = 2 ** (self.nestbit - 1)
                    n_low = 2 ** self.shiftbit  # Extra 1-bit for integer weights_low's ranges

                    self.scale_nest = self.scale / 2 ** self.shiftbit
                    self.zero_point_nest = self.zero_point / 2 ** self.shiftbit

                    if self.adaround:
                        self.quant_high_int = self.adaptive_round(
                            torch.clamp(quant_tensor / 2 ** self.shiftbit, -n_high, n_high - 1), -n_high, n_high - 1)
                        self.quant_low_int = torch.clamp(self.quant_int - self.quant_high_int * 2 ** self.shiftbit,
                                                         -n_low, n_low - 1)
                    else: # rounding-to-nearest
                        self.quant_high_int = torch.clamp((self.quant_int / 2 ** self.shiftbit).round(),
                                                          -n_high, n_high - 1)
                        self.quant_low_int = torch.clamp(self.quant_int - self.quant_shift_int_ * 2 ** self.shiftbit,
                                                         -n_low, n_low - 1)

                    if self.nest:
                        tensor_int_ = linear_dequantize(self.quant_high_int, self.scale_nest,
                                                        self.zero_point_nest, inplace=False)
                    else:
                        quant_recomp_int = torch.clamp(self.quant_high_int * 2 ** self.shiftbit + self.quant_low_int,
                                                          -n, n - 1)
                        tensor_int_ = linear_dequantize(quant_recomp_int, self.scale, self.zero_point, inplace=False)
...
class LinearQuantizer(nn.Module):
    ...
    """
    def __init__(self, mode=None, wbit=None, abit=None, args=None, nestbit=None, nest=None, adaround=None):
        """
        ...
        assert mode is not None,'Quantizer is not initilized!'
        self.quant_input  = TensorQuantizer(mode=mode, bit=abit, is_signed=False, is_enable=True, args=args,
                                            is_input=True)
        self.quant_weight = TensorQuantizer(mode=mode, bit=wbit, is_signed=True, is_enable=True, args=args,
                                            nestbit=nestbit, nest=nest, adaround=adaround)
