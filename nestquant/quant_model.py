...
import copy
from nestquant.quant_modules import Conv2dQuantizer, LinearQuantizer
from nestquant.quant_utils import nestquant_args
...
def quantize_model(model):
    ...
    if type(model) == nn.Conv2d:
        # print(model)
        # print(nestquant_args)
        quant_mod = Conv2dQuantizer(**nestquant_args)
        quant_mod.set_param(model)
        return quant_mod
    elif type(model) == nn.Linear:
        # print(model)
        quant_mod = LinearQuantizer(**nestquant_args)
        quant_mod.set_param(model)
        return quant_mod
