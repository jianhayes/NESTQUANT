...
def main():
    ...
    parser.add_argument('--savepath', default='./nestquant/qmodel/', type=str, help='save model')
    ...
    parser.add_argument('--nestquant', action='store_true', help='save packbits nestquant quantized model')
    ...
    if args.nestquant:
        ...
        quantized_DNN_model_nestquant = nest_quantized_model(quant_model)

        if not os.path.exists(args.savepath):
            os.makedirs(args.savepath)
        savepath = args.savepath + "{}_nestquant_W{}A{}Nest{}.pth".format(args.model, args.wbit, args.abit, args.nestbit)

        torch.save(quantized_DNN_model_nestquant.state_dict(), savepath)

        quantized_DNN_model_nestquant.load_state_dict(torch.load(savepath))

        if args.eval_nestquant:
            quantized_DNN_model_nestquant.cuda()
            ...
            ## Validation
            test(quantized_DNN_model_nestquant, testloader)
